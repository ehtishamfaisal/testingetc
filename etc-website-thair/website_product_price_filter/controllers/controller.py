...
class WebsiteSale(WebsiteSale):
    ...
    def product(self, product, category='', search='', **kwargs):
        ...
        res.qcontext['finish'] = request.env['table.finish'].sudo().search([])
        return res
    ...
    def cart_update(self, product_id, add_qty=1, set_qty=0, **kw):
        ...
        if line_id and finish:
            ...
            line_id.sudo().finish_id = int(finish) 
...
#Table Style Attribute =====================================================
    ...
    def _get_search_domain(self, search, category, attrib_values):
        ...
        url_path = url[1] if url[1] else ''
        page_path = str('/' + url[2] if len(url) > 3 else '')
        if str('/'+str(url_path) + str(page_path)) in ['/shop','/shop/page']:
            domain += [('wk_type','=','ot')]
        return domain
